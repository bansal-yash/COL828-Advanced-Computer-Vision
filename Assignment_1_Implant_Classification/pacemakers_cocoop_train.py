...
def train_stage_1(
    ...
    best_val_acc = 0.0
    best_model_path = os.path.join(args.save_dir, f"cocoop_stage_1.pth")
...
def train_stage_2(
    ...
    best_val_acc = 0.0
    best_model_path = os.path.join(args.save_dir, f"cocoop_stage_2.pth")
...
def train_cocoop(epochs_stage_1, epochs_stage_2):
    ...
    model_stage_1 = CLIP_Cocoop(manufacturers, clip_model_stage_1)
    ...
    for name, param in model_stage_1.named_parameters():
        if "prompt_learner.ctx" in name or "prompt_learner.meta_net" in name:
    ...
    model_stage_2 = CLIP_Cocoop(all_classes, clip_model_stage_2)
    ...
    with torch.no_grad():
        for cls_idx, cls_name in idx_to_class.items():
            ...
            model_stage_2.prompt_learner.ctx[cls_idx].copy_(
                manufacturer_prompts[manu_idx]
            )

        for (n1, p1), (n2, p2) in zip(
            model_stage_1.prompt_learner.meta_net.named_parameters(),
            model_stage_2.prompt_learner.meta_net.named_parameters(),
        ):
            p2.copy_(p1.data)
...
if __name__ == "__main__":
    ...
    train_cocoop(epochs_stage_1=50, epochs_stage_2=150)
